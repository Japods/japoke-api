...
const DELIVERY_COST_TEMPLATE = {
  name: 'delivery_cost',
  buildBody: (customerName, orderNumber, amountBs) => [
    { type: 'body', parameters: [
      { type: 'text', text: customerName },
      { type: 'text', text: orderNumber },
      { type: 'text', text: amountBs },
    ] },
  ],
};
...
export async function notifyDeliveryCost(order, amountBs) {
  if (!isEnabled()) {
    console.log(`[WhatsApp] Desactivado - no se envia costo delivery para ${order.orderNumber}`);
    return null;
  }
  ...
  if (usage.autoPaused) {
    console.warn(`[WhatsApp] Limite mensual alcanzado. Mensaje bloqueado para ${order.orderNumber}`);
    await WhatsAppLog.create({
      phone, month, orderNumber: order.orderNumber,
      template: DELIVERY_COST_TEMPLATE.name, status: 'blocked',
    });
    return null;
  }

  const customerName = order.customer.name.split(' ')[0];
  const formattedBs = Number(amountBs).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' Bs';
  const components = DELIVERY_COST_TEMPLATE.buildBody(customerName, order.orderNumber, formattedBs);
  const maskedPhone = phone.slice(0, 4) + '****' + phone.slice(-3);

  try {
    const result = await sendTemplateMessage(phone, DELIVERY_COST_TEMPLATE.name, components);
    await WhatsAppLog.create({
      phone, month, orderNumber: order.orderNumber,
      template: DELIVERY_COST_TEMPLATE.name, status: 'sent',
    });
    console.log(`[WhatsApp] Enviado: ${DELIVERY_COST_TEMPLATE.name} (${formattedBs}) → ${maskedPhone} (pedido ${order.orderNumber})`);
    return result;
  } catch (err) {
    await WhatsAppLog.create({
      phone, month, orderNumber: order.orderNumber,
      template: DELIVERY_COST_TEMPLATE.name, status: 'failed',
    });
    console.error(`[WhatsApp] Error enviando ${DELIVERY_COST_TEMPLATE.name} → ${maskedPhone}: ${err.message}`);
    throw err;
  }
}
